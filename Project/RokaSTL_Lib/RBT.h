...
namespace rokaStl
{
	...
	{
	public:
		Node() :parent(nullptr), left(nullptr), right(nullptr), type(NodeType::None), data(-1), is_extra(false), key(), is_innerDelete(false)
		{

		}
		Node(NodeType _type) :parent(nullptr), left(nullptr), right(nullptr), type(_type), data(-1), is_extra(false), key(), is_innerDelete(false)
		{
		...
		void RBT_Delete(Node<Key, Value>** _node)
		{
			...
			if (parent->left == (*_node))
			{
				...
				if (sibling->type == NodeType::Black)
				{
					...
					////오른쪽만 Red
					//else 
						if (sibling->right->type == NodeType::Red)
					{
			...
			else
			{
				...
				if (sibling->type == NodeType::Black)
				{
					...
					////왼쪽만 Red
					//else 
					if (sibling->left->type == NodeType::Red)
					{
...
#pragma endregion
	public:
		...
		void innerClear() { is_InnerDelete = true; }
		void insert(std::pair<Key, Value> _pair)
		{
			Node<Key, Value>* node = new Node<Key, Value>(_pair.first, _pair.second, nilnode, NodeType::Red, is_InnerDelete);
			push_node(&root, node);
			if (node == nullptr)
				return;
			RBT_Insert(&node);
